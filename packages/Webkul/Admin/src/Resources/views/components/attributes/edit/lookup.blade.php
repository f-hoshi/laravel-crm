...
@pushOnce('scripts')
    ...
    <script type="module">
        app.component('v-lookup-component', {
            ...
            beforeUnmount() {
                // ステージのlookupコンポーネントのイベントリスナーをクリーンアップ
                if (this.attribute.code === 'lead_pipeline_stage_id') {
            ...
            methods: {
                ...
                search() {
                    ...
                    this.$axios.get(this.searchRoute, {
                            params: { query: this.searchTerm, pipeline_id: this.selectedPipelineId }
                        })
                        .then (response => {
                ...
                loadInitialData() {
                    ...
                    this.$axios.get(this.searchRoute, {
                            params: { query: '', pipeline_id: this.selectedPipelineId }
                        })
                        .then (response => {
                            this.searchedResults = response.data;
                        })
                ...
                onPipelineChanged(pipelineId) {
                    ...
                    // ステージのlookupコンポーネントの場合、選択をクリアして検索結果をリセット
                    if (this.attribute.code === 'lead_pipeline_stage_id') {
                        this.selectedItem = {
                            id: '',
                            name: ''
